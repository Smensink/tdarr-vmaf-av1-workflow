...
var plugin = function (args) {
    ...
    // ── Unified SQLite training store: selection/retry facts only ──
    // Terminal success/failure belongs exclusively to monitorTranscodeRetry, after FFmpeg,
    // final-size checks, and HDR metadata verification have completed.
    try {
        ...
        vmafdbL.upsertJob(_dbL, {
            ...
            source_file_size_mb: Number(learningData.source_file_size_mb) || null,
            projected_output_ratio_pct: Number(learningData.selected_projected_output_ratio_pct) || null,
            projected_size_reduction_pct: Number(learningData.projected_size_reduction_pct) || null,
            // Preserve measurement-contract provenance even if the earlier export
            // step failed and this partial upsert has to create the job row.
        ...
        args.jobLog('SQLite training store: selection/retry facts attached to job ' + _jobIdL);
    } catch (dbErrL) {
        args.jobLog('WARNING: SQLite outcome write failed (non-fatal): ' + (dbErrL && dbErrL.message ? dbErrL.message : String(dbErrL)));
    }

    // Legacy EMA is diagnostic-only and disabled by default; SQLite similarity models are authoritative.
    var emaEnabled = args.inputs.emaEnabled === true || args.inputs.emaEnabled === 'true';
    if (emaEnabled) try {
        ...
        args.jobLog('Legacy EMA update disabled; SQLite training store is authoritative.');
    }

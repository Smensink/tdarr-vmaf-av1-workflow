...
var details = function () { return ({
    ...
    inputs: [
        ...
        },
        {
            ...
            defaultValue: '',
            inputUI: { type: 'text' },
            tooltip: 'Optional Plex base URL (e.g., http://host.docker.internal:32400)'
        },
        {
            label: 'Plex Token',
            name: 'plexToken',
...
var plugin = function (args) {
    ...
    var plexUrl = (process.env.TDARR_PLEX_URL || args.inputs.plexUrl || '').trim();
    var plexToken = (process.env.TDARR_PLEX_TOKEN || args.inputs.plexToken || '').trim();
    var plexSections = (args.inputs.plexSections || '').trim();
    var tmdbApiKey = (process.env.TDARR_TMDB_API_KEY || args.inputs.tmdbApiKey || '').trim();
    var tvdbApiKey = (process.env.TDARR_TVDB_API_KEY || args.inputs.tvdbApiKey || '').trim();
    var logMetadata = args.inputs.logMetadata !== false && args.inputs.logMetadata !== 'false';

    function redactSecrets(value) {
        return String(value || '')
            .replace(/([?&](?:api_key|X-Plex-Token)=)[^&\s]+/gi, '$1[REDACTED]')
    ...
    function plexSearch(title, year) {
        ...
        var commonQuery = '&X-Plex-Client-Identifier=' + clientId +
        ...
        function attempt(urlToTry) {
            if (logMetadata) {
                args.jobLog('Plex search: ' + redactSecrets(urlToTry) + ' (credential sent in header)');
            }
    ...
    function tmdbSearch(title, year) {
        ...
        if (logMetadata) {
            args.jobLog('TMDB search: ' + redactSecrets(searchUrl));
        }
    ...
    return runLookup(0).then(function(meta) {
        ...
        args.jobLog('Metadata fetch failed: ' + redactSecrets(err.message));
        return {
            outputFileObj: args.inputFileObj,
            outputNumber: 1,
            variables: args.variables,

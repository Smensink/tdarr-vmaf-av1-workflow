...
var plugin = function (args) {
    ...
    if (result.released) {
        ...
        args.variables.vmafGpuPipelineLockReleased = true;
        args.variables.vmafGpuPipelineLockAcquired = false;
    } else if (result.reason === 'no lock owner found') {
        args.jobLog('GPU pipeline lock was already absent - treating release as complete.');
        args.variables.vmafGpuPipelineLockReleased = true;
        args.variables.vmafGpuPipelineLockAcquired = false;
    } else {
        ...
        throw new Error('Refusing to continue with an unreleased GPU pipeline lock: ' + result.reason);
    }

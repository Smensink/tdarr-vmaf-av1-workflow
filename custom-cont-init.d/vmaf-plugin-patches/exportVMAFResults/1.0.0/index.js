...
function optionalNonnegativeSeconds(value) {
    if (value === undefined || value === null || String(value).trim() === '') return undefined;
    var seconds = Number(value);
    return isFinite(seconds) && seconds >= 0 ? seconds : undefined;
}

function resolveAggregateEncodeTime(aggregate, fallbackSeconds) {
    var aggregateSeconds = optionalNonnegativeSeconds(aggregate && aggregate.encodeTimeSecTotal);
    if (aggregateSeconds !== undefined) return aggregateSeconds;
    return optionalNonnegativeSeconds(fallbackSeconds);
}

function buildPairedCqAuthorityRecord(variables) {
    variables = variables || {};
    var outcome = variables.vmafPairedCqActingOutcome;
    if (!outcome || Number(outcome.schema) !== 4) return null;
    var identity = outcome.measurementIdentity || {};
    var requiredIdentity = ['metricContractId', 'referenceContractId', 'encoderProfileId',
        'measurementSubsample', 'policyId', 'inferenceAuthorityContractId'];
    var identityComplete = requiredIdentity.every(function (key) {
        var value = identity[key];
        return value !== null && value !== undefined && String(value).trim() !== '';
    });
    var holdout = variables.vmafAuthoritativeHoldoutOutcome || null;
    var holdoutExact = !!(holdout && holdout.directlyMeasured === true &&
        holdout.metricContractId === identity.metricContractId &&
        holdout.referenceContractId === identity.referenceContractId &&
        (holdout.passed === true || holdout.passed === false));
    var margins = outcome.conservativeCiMargins || {};
    var marginsPositive = ['harmonic', 'p1', 'cambi'].every(function (key) {
        return isFinite(Number(margins[key])) && Number(margins[key]) >= 0;
    });
    var inferred = outcome.action === 'paired_cq_inferred_v1';
    var jobId = variables.vmafCanonicalJobId || variables.vmafJobId || variables.vmafRunId || null;
    return {
        schema: 4,
        metricMode: 'selector_authoritative_harmonic_v4_inference_authority',
        timestamp: new Date().toISOString(),
        decisionId: [jobId || 'unknown', outcome.previousParameterSetId || '',
            outcome.currentParameterSetId || '', outcome.action].join('|'),
        jobId: jobId,
        measurementIdentity: identity,
        decision: { action: inferred ? 'predicted_safe' : 'fallback_full', reasons: outcome.reasons || [] },
        eligibleForPromotion: inferred && identityComplete && holdoutExact && marginsPositive,
        falseSafe: inferred && holdoutExact ? holdout.passed === false : null,
        holdoutOutcome: holdoutExact ? holdout : null,
        measurementCounts: {
            measuredClipCount: Number(outcome.measuredClipCount) || 0,
            inferredClipCount: Number(outcome.inferredClipCount) || 0,
        },
        conservativeCiMargins: marginsPositive ? margins : null,
        confidence95Halfwidth: outcome.confidence95Halfwidth || null,
        maximumConfidence95Halfwidth: outcome.maximumConfidence95Halfwidth || null,
        estimatedMetricSecondsSaved: optionalNonnegativeSeconds(outcome.estimatedMetricSecondsSaved),
        realizedMetricSecondsSaved: null,
        exclusionReasons: [].concat(identityComplete ? [] : ['incomplete_measurement_identity'],
            holdoutExact ? [] : ['missing_or_identity_mismatched_measured_holdout'],
            marginsPositive || !inferred ? [] : ['nonpositive_or_missing_conservative_ci_margin']),
    };
}

function isOrdinaryMeasuredAggregate(aggregate) {
    if (!aggregate || typeof aggregate !== 'object') return false;
    if (aggregate.measurementDisposition === 'paired_cq_inferred_v1') return false;
    if (Array.isArray(aggregate.clipProvenance) && aggregate.clipProvenance.some(function (value) {
...
var plugin = function (args) {
    ...
    var pairedAuthorityRecord = buildPairedCqAuthorityRecord(args.variables);
    if (pairedAuthorityRecord) {
        try {
            fs.appendFileSync(pairedCqShadow.TELEMETRY_PATH,
                JSON.stringify(pairedAuthorityRecord) + '\n', 'utf8');
            args.variables.vmafPairedCqAuthorityTelemetry = pairedAuthorityRecord;
            args.jobLog('Paired-CQ schema-v4 authority telemetry: action=' +
                pairedAuthorityRecord.decision.action + ', promotionEligible=' +
                pairedAuthorityRecord.eligibleForPromotion);
        } catch (pairedTelemetryError) {
            args.jobLog('WARNING: paired-CQ authority telemetry write failed (non-fatal): ' +
                pairedTelemetryError.message);
        }
    }
    ...
    var sourceFilePath = inputFile._id || inputFile.file || inputFile.filePath || '';
    var fileMetadata = {
    ...
    try {
        ...
        // Derive the job id from the real file path + shared start-time nonce. Reassign both
        // variables so downstream terminal/shadow writers converge on the same canonical key.
        ...
        // Preserve the canonical source-row id across the final transcode. After FFmpeg, Tdarr's
        // inputFileObj points at the temporary output, whose path hashes to a different job id.
        args.variables.vmafCanonicalJobId = _jobId;
        args.variables.vmafJobId = _jobId;
        ...
        function _sizeMb(x) {
            ...
            // Tdarr file_size is normally bytes; tolerate callers that already provide MB.
            return n > 1024 * 1024 ? (n / 1024 / 1024) : n;
        }
        var _sourceSizeMb = _sizeMb(fileMetadata.fileSize);
        var _sourceBitrateMbps = Number(fileMetadata.videoBitrate) > 0 ? Number(fileMetadata.videoBitrate) / 1000000 : null;
        if ((!_sourceBitrateMbps || !isFinite(_sourceBitrateMbps)) && _sourceSizeMb && Number(fileMetadata.duration) > 0) {
            _sourceBitrateMbps = (_sourceSizeMb * 8) / Number(fileMetadata.duration);
        }
        vmafdb.upsertJob(_db, {
            ...
            reference_contract_id: args.variables.vmafReferenceContractId || null,
            metric_contract_id: args.variables.vmafMetricContractId || undefined,
            encoder_profile_id: args.variables.vmafEncoderProfileId || undefined,
            source_codec: fileMetadata.videoCodec,
            source_width: fileMetadata.videoWidth,
            source_height: fileMetadata.videoHeight,
            source_bitrate_mbps: _sourceBitrateMbps,
            source_file_size_mb: _sourceSizeMb,
            ...
            effective_frame_floor: args.variables.vmafMinFrameVMAF != null ? Number(args.variables.vmafMinFrameVMAF) : null,
            effective_cambi_limit: args.variables.vmafEffectiveCambiLimit != null ? Number(args.variables.vmafEffectiveCambiLimit) : null,
            selector_policy_version: args.variables.vmafSelectorPolicyVersion || 'selector-authoritative-v2',
        ...
        var _pts = [];
        var _spos = Array.isArray(args.variables.vmafSamplePositions) ? args.variables.vmafSamplePositions : null;
        var _segDur = Number(args.variables.vmafSegmentDuration) || null;
        var _encByPs = {};
        var _tr = args.variables.vmafTestResults || [];
        for (var _ti2 = 0; _ti2 < _tr.length; _ti2++) {
            var _t = _tr[_ti2] || {};
            if (_t.parameterSetId && isFinite(Number(_t.encodingTimeSeconds))) {
                _encByPs[_t.parameterSetId] = (_encByPs[_t.parameterSetId] || 0) + Number(_t.encodingTimeSeconds);
            }
        }
        for (var _ai = 0; _ai < aggregatedResults.length; _ai++) {
            var _ar = aggregatedResults[_ai] || {};
            // Cross-job reuse (2026-07-20): revived rows were measured by a PRIOR job and must
            // not be re-inserted under this job's id — a fresh created_at would defeat the
            // reuse age-gating and duplicate training rows on every requeue.
            if (_ar.reusedFromJobId) continue;
            // Paired-CQ acting rows are selector evidence with explicit inference provenance,
            // never measured training observations. Keep them out of sweep_points entirely.
            if (!isOrdinaryMeasuredAggregate(_ar)) continue;
            var _ps = _ar.parameterSet || {};
            // Per-row clip provenance: which sample indices were measured (aligned with clip_vmafs)
            // and their source positions/durations. Rows can differ in clip count (sequential stop),
            // so this is the only reliable way to pair clips across CQs in training data.
            var _cidx = Array.isArray(_ar.clipSampleIndices) ? _ar.clipSampleIndices : null;
            var _cstarts = (_cidx && _spos) ? _cidx.map(function (ix) {
                return (ix != null && _spos[ix] != null) ? Math.round(Number(_spos[ix]) * 10) / 10 : null;
            }) : null;
            var _cdurs = (_cidx && _segDur) ? _cidx.map(function () { return _segDur; }) : null;
            // The aggregate carries current-job encode timing across bracket/refinement rounds.
            // Older/in-flight payloads lack it, so retain the prior last-round lookup as fallback.
            var _enc = resolveAggregateEncodeTime(_ar, _encByPs[_ar.parameterSetId]);
            var _vt = Number(_ar.vmafTimeSecTotal);
            var _ct = Number(_ar.cambiTimeSecTotal);
            var _st = Number(_ar.ssimTimeSecTotal);
            var _work = (isFinite(_enc) ? _enc : 0) + (isFinite(_vt) ? _vt : 0) +
                (isFinite(_ct) ? _ct : 0) + (isFinite(_st) ? _st : 0);
            _pts.push({
                ...
                clip_vmaf_means: JSON.stringify(_ar.clipVmafMeans || []),
                clip_vmaf_p1s: JSON.stringify(_ar.clipVmafP1s || []),
                clip_cambis: _ar.clipCambis ? JSON.stringify(_ar.clipCambis) : null,
                clip_sample_indices: _cidx ? JSON.stringify(_cidx) : null,
                clip_time_starts: _cstarts ? JSON.stringify(_cstarts) : null,
                clip_durations: _cdurs ? JSON.stringify(_cdurs) : null,
                encode_time_sec: isFinite(_enc) ? _enc : null,
                vmaf_time_sec: isFinite(_vt) ? _vt : null,
                cambi_time_sec: isFinite(_ct) ? _ct : null,
                ssim_time_sec: isFinite(_st) ? _st : null,
                measured_work_time_sec: _work > 0 ? _work : null,
                measured_clip_count: parseInt(_ar.sampleCount) || null,
                measured_clip_seconds: (_segDur && _ar.sampleCount) ? _segDur * _ar.sampleCount : null,
                timing_source: _work > 0 ? 'live_plugin' : null,
                measurement_disposition: _ar.measurementDisposition || 'measured_full_current_contract'
            });
        }

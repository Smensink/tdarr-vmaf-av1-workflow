...
var plugin = function (args) {
    ...
    var rejectionReasons = require('../../_lib/rejectionReasons.js');
    ...
    function buildConstraintAwareLowerRetryPlan(results, testedCQMin, testedCQSet, cqStepSize, minVMAF, minFrameVMAF) {
        ...
        if (minFrameVMAF > 0) {
            ...
            if (lowFloor !== null && lowFloor < minFrameVMAF) {
                // 1%-low is noisier than mean VMAF and can look nearly flat across the
                // first failed bracket. A loose 12-CQ extrapolation cap can overreact
                // badly (Battle Royale: CQ24/29/34 failed 1%-low, projected binding
                // CQ12, then retried only CQ16/18 even though mean VMAF boundary was
                // CQ22.9 and the later fractional selector landed near CQ21.6). Keep
                // this retry close to the measured edge and let later retries walk lower
                // if needed.
                applyBinding(estimateLowerCrossing(byCQ, floorMetric, minFrameVMAF, true, 4), '1%low');
            }
        }
    ...
    if (sweepRetryTriggered) {
        ...
        if (overrideCQMin !== undefined && overrideCQMax !== undefined) {
            ...
            // Reset attempt-scoped refinement state: a refinement round forces vmafSubsample='1'
            // (full-rate VMAF) which would otherwise stick for the WHOLE retry sweep (4x VMAF
            // cost on coarse rounds), and the refinement probe budget should renew per sweep.
            if (args.variables.vmafSubsampleAuto !== false) delete args.variables.vmafSubsample;
            delete args.variables.vmafActiveRefineCount;
    ...
    // Infeasible-file cooldown (2026-07-20): the sweep was skipped entirely because this exact
    // file already failed terminally under the current feasibility-policy era. Never schedule
    // retries for a cooldown skip — route straight to the give-up/keep-original path.
    if (args.variables.vmafCooldownSkip) {
        args.jobLog('Infeasible-file cooldown active (prior terminal failure '
            + (args.variables.vmafCooldownSkip.priorJobId || 'unknown') + '); no retry scheduled.');
        args.variables.vmafTranscodeGaveUp = true;
        args.variables.vmafSweepRetriesExhausted = true;
        return {
            outputFileObj: args.inputFileObj,
    ...
    if (selectOutput === 2) {
        ...
        if (highestTestedVMAF < minVMAF || lowestValidCQ === null || !bestParams) {
            ...
            if (isAvgVmafFailure) {
                ...
                // - UNLESS the lowest tested CQ was rejected for insufficient size benefit
                // (max-ratio guard): lower CQ only increases output size, so retrying downward
                // can never help and the file is size-infeasible at quality-passing CQs.
                var _rejList = args.variables.vmafRejectedResults || [];
                var _minRejCq = null, _minRejReason = '';
                for (var _rr = 0; _rr < _rejList.length; _rr++) {
                    var _rEnt = _rejList[_rr];
                    var _rCq = _rEnt ? Number(_rEnt.cq) : NaN;
                    if (isFinite(_rCq) && (_minRejCq === null || _rCq < _minRejCq)) {
                        _minRejCq = _rCq;
                        _minRejReason = String(_rEnt.reason || '');
                    }
                }
                // Match every generation of the selector's size-cap rejection wording via the
                // shared _lib/rejectionReasons.js contract (pre-2026-07-20 'insufficient size
                // benefit', and the demoted-gate era's 'emergency cutoff' / 'legacy cap' reasons
                // - see evaluateSizeGate). Centralised 2026-07-21 so a future wording change only
                // needs to be reconciled in one place instead of duplicated per consumer.
                if (rejectionReasons.isSizeTerminalReason(_minRejReason)) {
                    sizeBenefitTerminal = true;
                }

                args.jobLog('');
                args.jobLog('⚠ Average VMAF (' + highestTestedVMAF.toFixed(2) + ') meets threshold but selection guards rejected all candidates');
                if (!sizeBenefitTerminal) {
            ...
            // CQ16 quality-ceiling terminal check (see cq16QualityCeilingTerminal below).
            // Kill switch: args.variables.vmafCq16GiveUp === false.
            var cq16Terminal = (args.variables.vmafCq16GiveUp !== false)
                ? cq16QualityCeilingTerminal(aggregatedResults, minVMAF, minFrameVMAF)
                : null;

            if (cq16Terminal) {
                shouldRetry = false;
                retryReason = 'Quality ceiling measured at CQ ' + cq16Terminal.cq + ' still fails thresholds by >0.5'
                    + ' (VMAF ' + cq16Terminal.vmaf.toFixed(2) + ' vs ' + minVMAF
                    + (cq16Terminal.p1 !== null && minFrameVMAF > 0 ? ', 1%-low ' + cq16Terminal.p1.toFixed(2) + ' vs ' + minFrameVMAF : '')
                    + ') - no lower CQ exists, giving up without further descent';
                args.jobLog('⚠ ' + retryReason);
                args.variables.vmafCq16GiveUpFired = true;
            } else if (sizeBenefitTerminal) {
                shouldRetry = false;
                retryReason = 'Lowest quality-passing CQ ' + _minRejCq + ' already projects insufficient size benefit - lower CQ only increases output size';
                args.jobLog('⚠ ' + retryReason);
            } else {
    ...
    if (shouldRetry && newCQMin !== null && newCQMax !== null) {
        ...
        args.variables.vmafAggregatedResults = [];
        args.variables.vmafBestParameters = null;
        // Reset attempt-scoped refinement state (see identical reset above).
        if (args.variables.vmafSubsampleAuto !== false) delete args.variables.vmafSubsample;
        delete args.variables.vmafActiveRefineCount;
...
// ── CQ16 quality-ceiling terminal check (2026-07-20) ──
// VMAF and 1%-low are monotone non-increasing in CQ, so if the encoder's maximum-quality
// point (CQ<=16.5) has been measured and still fails the thresholds by more than 0.5 points,
// no CQ in [16,51] can pass and further descent is pure waste. Historical replay: 577 jobs
// measured CQ<=16.5; a 0.5-pt margin leaves 3 (0.5%) cross-attempt-noise contradictions, and
// the failure direction is conservative (keep-original). Returns the terminal point or null.
function cq16QualityCeilingTerminal(aggregatedResults, minVMAF, minFrameVMAF) {
    var points = [];
    var results = aggregatedResults || [];
    for (var i = 0; i < results.length; i++) {
        var result = results[i];
        var cq = result && result.parameterSet ? Number(result.parameterSet.quality) : NaN;
        if (!isFinite(cq) || cq > 16.5) continue;
        // Explicit null/undefined/'' guard: Number(null) is 0, which would make an
        // UNMEASURED point look like a catastrophic quality failure.
        if (result.avgVMAF === null || result.avgVMAF === undefined || result.avgVMAF === '') continue;
        var vmafHarmonic = Number(result.avgVMAF);
        if (!isFinite(vmafHarmonic)) continue;
        var p1 = (result.vmafP1Low !== null && result.vmafP1Low !== undefined && isFinite(Number(result.vmafP1Low)))
            ? Number(result.vmafP1Low) : null;
        var hardFail = (vmafHarmonic < Number(minVMAF) - 0.5)
            || (Number(minFrameVMAF) > 0 && p1 !== null && p1 < Number(minFrameVMAF) - 0.5);
        points.push({ cq: cq, vmaf: vmafHarmonic, p1: p1, hardFail: hardFail });
    }
    if (points.length > 0 && points.every(function (point) { return point.hardFail; })) {
        return points[0];
    }
    return null;
}
...
exports._test = {
    cq16QualityCeilingTerminal: cq16QualityCeilingTerminal,
};

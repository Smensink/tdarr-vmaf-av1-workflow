...
function weightForPoint(src, row, opts) {
  ...
  // Cross-reference-contract rows are permitted only as explicitly
  // uncertainty-weighted planning evidence.  The bridge stamps this field on
  // sanitized clones; ordinary same-contract rows omit it and retain weight 1.
  // Invalid or over-unity values fail closed so a malformed adapter cannot
  // accidentally promote legacy evidence to canonical strength.
  if (Object.prototype.hasOwnProperty.call(row || {}, 'reference_contract_weight')) {
    var contractWeight = Number(row.reference_contract_weight);
    if (!isFinite(contractWeight) || contractWeight <= 0 || contractWeight > 1) return 0;
    w *= contractWeight;
  }
...
/**
 * The live selector applies targetVmaf to the average of each clip's harmonic
 * VMAF score.  Sweep rows also retain the arithmetic frame mean for diagnostics,
 * but using that (usually higher) value to estimate CQ can propose a quality
 * level that the live harmonic gate rejects.  Prefer a structurally-valid
 * harmonic value and fall back to the arithmetic value for legacy rows.
 *
 * Some early backfilled rows contain column-shifted harmonic values below their
 * own recorded minimum.  Those cannot be real aggregate scores, so they must
 * not become predictor inputs.
 */
function maximumVmafObservation(row) {
  row = row || {};
  var contractId = String(row.metric_contract_id || row.metricContractId || '');
  return contractId.indexOf('vmaf-v1.0.16') !== -1 ? 110 : 100;
}

function primaryVmaf(row) {
  row = row || {};
  ...
  var harmonic = finiteMetric(row.vmaf_harmonic_mean);
  var mean = finiteMetric(row.vmaf_mean != null ? row.vmaf_mean : row.v);
  var min = finiteMetric(row.vmaf_min);
  var max = finiteMetric(row.vmaf_max);
  var maximumObservation = maximumVmafObservation(row);
  var harmonicValid = harmonic !== null && isFinite(harmonic) && harmonic >= 0 && harmonic <= maximumObservation;
  if (harmonicValid && min !== null && isFinite(min) && harmonic < min - 0.001) harmonicValid = false;
  if (harmonicValid && max !== null && isFinite(max) && harmonic > max + 0.001) harmonicValid = false;
  // For positive VMAF values the harmonic mean cannot exceed the arithmetic
  // mean, apart from insignificant serialization/rounding noise.
  if (harmonicValid && mean !== null && isFinite(mean) && harmonic > mean + 0.01) harmonicValid = false;
  if (harmonicValid) return harmonic;
  return mean !== null && isFinite(mean) && mean >= 0 && mean <= maximumObservation ? mean : null;
}

/**
 * Select CQ subject to quality constraints, maximizing compression (highest feasible cq).
 *
 * curveRows: rows from vmafdb.getSimilarSweepCurves (each: cq,
 *            vmaf_harmonic_mean/vmaf_mean, vmaf_p1_low,
...
function selectCQ(curveRows, src, constraints, opts) {
  ...
  for (var i = 0; i < curveRows.length; i++) {
    ...
    if (w <= 0) continue;
    totalW += w; nRows++;
    var primary = primaryVmaf(row);
    if (primary != null) vmafPts.push({ cq: cq, val: primary, w: w });
...
function fitLogCeiling(points, ceiling) {
  if (!isFinite(Number(ceiling)) || Number(ceiling) <= 0) {
    ceiling = points.some(function (point) { return maximumVmafObservation(point) === 110; }) ? 110 : 100;
  } else {
...
function curveOptimalAtTarget(points, target) {
  var pts = [];
  for (var i = 0; i < points.length; i++) {
    var cq = Number(points[i].cq), v = primaryVmaf(points[i]);
    if (isFinite(cq) && v != null) pts.push({ cq: cq, v: v });
  }
...
function bindingTargetCQ(pts, constraints) {
  ...
  if (constraints.targetVmaf != null) {
    var fv = fitLogCeiling(pts.map(function (p) {
      return { cq: p.cq, vmaf_harmonic_mean: p.vmaf_harmonic_mean, vmaf_mean: p.vmaf_mean,
        v: p.v, metric_contract_id: p.metric_contract_id || p.metricContractId };
    }));
  ...
  if (constraints.vmafFloor != null) {
    ...
    if (pp.length >= 2) { var fp = fitLogCeiling(pp.map(function (p) { return { cq: p.cq,
      vmaf_mean: p.vmaf_p1_low, metric_contract_id: p.metric_contract_id || p.metricContractId }; })); if (fp) { var c2 = fp.cqAt(constraints.vmafFloor); if (c2 != null && isFinite(c2)) limits.push({ cq: c2, k: 'vmaf_p1_low' }); } }
  }
...
function constraintAwareOptimum(rows, target, opts) {
  ...
  if (useP1 || useCambi) {
    var pts = rows.map(function (r) {
      return {
        cq: Number(r.cq), vmaf_harmonic_mean: r.vmaf_harmonic_mean,
        vmaf_mean: r.vmaf_mean, v: primaryVmaf(r),
        metric_contract_id: r.metric_contract_id || r.metricContractId,
 ...
 *
 *   measured: [{cq, vmaf_harmonic_mean|vmaf_mean}]
 *   constraints:{targetVmaf}   opts:{centerCq, priorSlope, toleranceVmaf, cqStep}
 */
function nextSweepCQ(measured, constraints, opts) {
  ...
  for (var i = 0; i < measured.length; i++) {
    var m = measured[i], cq = Number(m.cq), v = primaryVmaf(m);
...
function selectCQFromDb(db, vmafdb, src, constraints, opts) {
  ...
  var queryOpts = {
    ...
    codec: opts.hardCodecFilter,
    referenceContractId: opts.referenceContractId,
    metricContractId: opts.metricContractId,
    encoderProfileId: opts.encoderProfileId
  };
  var curves = vmafdb.getSimilarSweepCurves(db, src, queryOpts);
  ...
  if (src && src.file_path) {
    var same = vmafdb.getSameFileSweepCurves(db, src.file_path, {
      limit: 2000,
      referenceContractId: opts.referenceContractId,
      metricContractId: opts.metricContractId,
      encoderProfileId: opts.encoderProfileId
...
function sampleStatsFromDb(db, vmafdb, src, opts) {
  opts = opts || {};
  var curves = vmafdb.getSimilarSweepCurves(db, src, {
    limit: opts.limit || 20000,
    referenceContractId: opts.referenceContractId,
    metricContractId: opts.metricContractId,
    encoderProfileId: opts.encoderProfileId
...
module.exports = {
  ...
  maximumVmafObservation: maximumVmafObservation,
  primaryVmaf: primaryVmaf,
  selectCQ: selectCQ,

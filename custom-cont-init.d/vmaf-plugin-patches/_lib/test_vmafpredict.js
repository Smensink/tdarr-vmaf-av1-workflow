'use strict';
var P;
try { P = require('/custom-cont-init.d/vmaf-plugin-patches/_lib/vmafpredict.js'); }
catch (eRequire) { P = require('./vmafpredict.js'); }
var pass = 0, fail = 0;
...
// ── Real DB smoke test ──
// Explicit opt-in is required because openDb() applies schema migrations. A
// routine synthetic test must never mutate the production training database.
if (process.env.VMAF_PREDICT_TEST_LIVE_DB === '1') try {
  var vmafdb;
  try { vmafdb = require('/custom-cont-init.d/vmaf-plugin-patches/_lib/vmafdb.js'); }
  catch (eDbRequire) { vmafdb = require('./vmafdb.js'); }
  var db = vmafdb.openDb(process.env.VMAF_PREDICT_TEST_DB || undefined);
  ...
  db.close();
} catch (e) { console.log('REAL DB test failed:', e.message); fail++; }
else console.log('\nREAL DB test skipped (set VMAF_PREDICT_TEST_LIVE_DB=1 to opt in).');

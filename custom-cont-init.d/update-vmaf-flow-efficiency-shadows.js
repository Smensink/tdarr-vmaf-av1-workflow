...
const backupPath = '/app/server/Tdarr/DB2/SQL/database.db.bak-efficiency-shadows-20260713';
const flowId = 'YR5PZ1QaD';
const apply = process.argv.includes('--apply');
const desired = {
  testEncodingParameters: { sameTitleEmptyBandShadow: 'true' },
  calculateVMAF: {
    pairedCqShadow: 'true',
...
if (!apply) {
  console.log(changes.length ? `Dry run:\n${changes.join('\n')}` : 'Dry run: efficiency shadow controls already current');
  process.exit(0);
}
if (!changes.length) {
  console.log('Live flow efficiency shadow controls already current');
  process.exit(0);
}
